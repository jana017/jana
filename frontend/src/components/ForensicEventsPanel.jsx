...
export default function ForensicEventsPanel({ forensic }) {
  ...
  return (
    <div className="mb-6 rounded-xl border border-slate-200 bg-white overflow-hidden" data-testid="analyzer-forensic-panel">
      <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-slate-200 bg-slate-50 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="inline-flex rounded border border-slate-200 bg-white p-0.5">
            <button
              data-testid="forensic-view-table"
              onClick={() => setView("table")}
              className={`inline-flex items-center gap-1 px-2 py-1 text-[11px] font-semibold rounded transition-colors ${
                view === "table" ? "bg-[#2E7DF5] text-white" : "text-slate-600 hover:bg-slate-100"
              }`}
            ><Table className="w-3 h-3" /> Table</button>
            <button
              data-testid="forensic-view-timeline"
              onClick={() => setView("timeline")}
              className={`inline-flex items-center gap-1 px-2 py-1 text-[11px] font-semibold rounded transition-colors ${
                view === "timeline" ? "bg-[#2E7DF5] text-white" : "text-slate-600 hover:bg-slate-100"
              }`}
            ><Clock className="w-3 h-3" /> Timeline</button>
          </div>
      ...
      {view === "timeline" ? (
        <div className="p-3"><InvestigationTimeline events={filtered} /></div>
      ) : (
